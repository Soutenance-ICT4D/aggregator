...
import com.sharepay.aggregator.shared.gateway.config.OrangeMoneyProperties;
...
public class OrangeMoneyGateway implements PaymentGateway {
    ...
    private static final String PROVIDER_CODE = "ORANGE_MONEY_CM";
    ...
    private final OrangeMoneyProperties props;
    ...
    public GatewayPayInResponse initiatePayIn(GatewayPayInRequest request) {
        Map<String, Object> body = Map.of(
                ...
                "callback_url",   props.getNotifUrl(),
        ...
        try {
            ...
            if (!"LOGIN_SUCCESS".equals(status)) {
                throw new BusinessException("Authentification échouée : " + status,
                        HttpStatus.BAD_GATEWAY, "ORANGE_AUTH_ERROR");
            }
            return root.path("data").asText();
        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            log.error("[Orange] Erreur d'authentification : {}", e.getMessage());
            throw new BusinessException("Authentification Orange Money échouée.", HttpStatus.BAD_GATEWAY, "ORANGE_AUTH_ERROR");
        }
    }
